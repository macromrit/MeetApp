...
def newTavilySearch(query: str, tavily_client: TavilyClient = tavily_client) -> str:
    try:
        # Step1 - tavily_client has already been loaded
        # Step 2. Executing a search query
        responses = tavily_client.search(
        ...
        # Step 3. Return the results
        # return [eval(response) for response in responses]
        return [response['content'] for response in responses['results']], [response['url'] for response in responses['results']]
...
if __name__ == '__main__':
    ...
    # testing Tavily API
    realtime_context = newTavilySearch("average fertility rate in India")
